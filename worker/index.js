...
async function requireAuth(c, next) {
    ...
    // 后台接口不再接受固定字符串 token，而是校验登录时写入 D1 的会话记录。
...
app.post('/api/auth/login', async (c) => {
    ...
    // 会话默认保留 7 天，既减少频繁登录，也避免永久有效的后台凭证。
    await c.env.DB.prepare("DELETE FROM sessions WHERE expires_at <= datetime('now') OR user_id = ?").bind(user.id).run();
    await c.env.DB.prepare("INSERT INTO sessions (token, user_id, expires_at) VALUES (?, ?, datetime('now', '+7 days'))")
        .bind(token, user.id).run();
    await logAction(c.env.DB, user.id, 'login', `User ${username} logged in`);
    return c.json({ success: true, token, user: { id: user.id, username: user.username, role: user.role } });
});
...
app.post('/api/health-check', requireAuth, async (c) => {
    ...
    await Promise.all(urls.map(async (url) => {
        ...
        try {
            const resp = await fetch(url, {
                method: 'GET',
                headers: { 'User-Agent': 'PaoGe-HealthCheck/1.0' },
                redirect: 'follow',
            });
